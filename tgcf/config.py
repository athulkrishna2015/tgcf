...
class LoginConfig(BaseModel):
    ...
    USERNAME: str = ""
    SESSION_STRING: str = ""
    BOT_TOKEN: str = ""  # Also used as helper bot sender in past mode when user_type=1
    ...
    try:
        if stg.CONFIG_TYPE == 1:
            with open(CONFIG_FILE_NAME, encoding="utf8") as file:
                cfg = Config.parse_raw(file.read())
                cfg.forwards = [
                    Forward(**f) if isinstance(f, dict) else f for f in cfg.forwards
                ]
                return cfg
...
async def load_from_to(
    ...
    for forward in forwards:
        if isinstance(forward, dict):
            ...
            dest = forward.get("dest")
        else:
            use_this = forward.use_this
...
def read_db():
    ...
    cfg.forwards = [Forward(**f) if isinstance(f, dict) else f for f in cfg.forwards]
    return cfg
